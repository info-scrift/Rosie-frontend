...
import { getCompanyAnalytics, getCompanyDashboard } from "@/services/companyservice";
...
const ClientAnalytics = () => {
  ...
  const [analytics, setAnalytics] = useState<Awaited<ReturnType<typeof getCompanyAnalytics>> | null>(null);
  const [dash, setDash] = useState<Awaited<ReturnType<typeof getCompanyDashboard>> | null>(null);

  useEffect(() => {
    (async () => {
      try {
        const [a, d] = await Promise.all([getCompanyAnalytics(), getCompanyDashboard()]);
        setAnalytics(a);
        setDash(d);
      } catch (e: any) {
  ...
  const applicationData = analytics!.applicationSeries.map(s => ({
    name: s.name, applications: s.applications, views: undefined // if you track views later
  }));
  const jobPerformanceData = analytics!.jobPerformance.map(j => ({
    name: j.name, applications: j.applications, conversion: j.conversion
  }));
  ...
  const topCandidates = analytics!.topCandidates.map(a => ({
    id: a.id,
    name: a.applicant_id, // TODO: replace with real name if you join profiles
    role: "", // you can enrich via job_id -> job title if needed
    score: a.score ?? 0,
    status: a.status,
    appliedDate: a.created_at
  }));

  // Use dash?.totals to fill your top StatsCard numbers.
};
