...
// import { listCompanyCandidates } from "@/services/companyservice";
...
//   // map UI filter values to API statuses
...
//       setLoading(true);
//       try {
//         const resp = await listCompanyCandidates({
//           status: statusMap[statusFilter],
//           q: searchTerm || undefined,
//           limit: 100,
//           offset: 0,
//         });
//         // Adapt to your table’s expected shape (add placeholders where we don’t have name/email yet)
//         const mapped = resp.rows.map(r => ({
//           id: r.id,
//           name: r.applicant_id,      // TODO: join to applicant profile for real name
//           email: "",
//           phone: "",
//           location: "",
//           role: "",                  // could be enriched by fetching job by r.job_id
//           experience: "",
//           skills: [],
//           score: r.score ?? 0,
//           status: r.status,
...
//           lastActivity: "",
//           avatar: (r.applicant_id || "AP").slice(0,2).toUpperCase(),
//           salary: "",
//         }));
//         setRows(mapped);
//       } catch (e: any) {
...
//   // Then pass `rows` into your <DataTable data={rows} … />
// };
...
export default function ClientCandidates(): JSX.Element {
  ...
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [roleFilter, setRoleFilter] = useState("all"); // kept for UI parity
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const mapped: CandidateRow[] = (resp?.rows ?? []).map((r: any) => ({
          id: r.id,
          name: r.applicant_id || "Applicant",
          email: "",
          phone: "",
          location: "",
          role: "", // enrich later with /jobs/:id if needed
          experience: "",
          skills: [],
          score: r.score ?? 0,
          status: r.status ?? "Under Review",
          appliedDate: r.created_at ? new Date(r.created_at).toISOString() : "",
          lastActivity: "",
          avatar: (r.applicant_id || "AP").slice(0, 2).toUpperCase(),
          salary: "",
        }));
  ...
  const statusStats = useMemo(() => {
    return [
      ...
      { label: "Under Review", value: rows.filter((c) => c.status === "Under Review").length, color: "blue" },
      {
        label: "Interviews",
        value: rows.filter((c) => c.status.includes("Interview") || c.status.includes("Phone")).length,
        color: "blue",
      },
      { label: "Hired", value: rows.filter((c) => c.status === "Hired").length, color: "blue" },
    ];
  }, [rows]);

  // columns shape kept compatible with your DataTable
  const columns: any[] = [
    {
      key: "name",
      title: "Candidate",
      sortable: true,
      render: (value: string, row: CandidateRow) => (
        <div className="flex items-center space-x-3">
          <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center relative">
            <span className="text-blue-600 font-semibold">{row.avatar}</span>
            {row.score >= 90 && (
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <div>
          <h1 className="text-4xl font-display font-bold text-gradient-primary mb-2">Candidate Management</h1>
          <p className="text-lg text-slate-600">Review, evaluate, and manage your job applicants</p>
        </div>
        <div className="flex items-center space-x-3">
          <Button variant="outline" className="executive-button-secondary focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
            <Download className="w-4 h-4 mr-2" />
      ...
      {loading && <div className="text-sm text-muted-foreground">Loading candidates…</div>}
