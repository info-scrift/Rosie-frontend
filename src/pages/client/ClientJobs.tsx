...
// import { listOwnJobs } from "@/services/companyservice";
...
//           applicants: 0, // optionally compute via /dashboard or /jobs/:id/applications
...
//   // Use `jobs` in your grid. Keep the rest of your UI as-is.
// };
...
export default function ClientJobs(): JSX.Element {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await listOwnJobs();
        const mapped: UiJob[] = (data ?? []).map((j: any) => ({
          id: j.id,
          title: j.title,
          department: j.industry || "General",
          location: j.location,
          ...
          salary:
            j.salary_min && j.salary_max
              ...
              : "—",
          applicants: 0, // hook up to /company/dashboard or /jobs/:id/applications if available
          status: j.is_active ? "Active" : "Paused",
  ...
  const filteredJobs = jobs.filter((j) =>
    [j.title, j.department, j.location, j.type].join(" ").toLowerCase().includes(searchTerm.toLowerCase())
  );
                ...
                <div className="flex items-center justify-between pt-4 border-t">
                  ...
                  <Link to={`/client/portal/jobs/${job.id}`}>
                    <Button
                      variant="outline"
                      size="sm"
                      className="opacity-0 group-hover:opacity-100 transition-all duration-200 bg-white border-slate-200 text-slate-700 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700"
                    >
                      View Details
                    </Button>
                  </Link>
