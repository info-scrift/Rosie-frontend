  ...
  const ProfilePage = () => {
      ...
      (async () => {
        try {
          ...
          const statusGuess =
            e?.status ??
            e?.response?.status ??
            (/\b401\b/.test(msg) ? 401 : undefined);

          const looksExpired =
            statusGuess === 401 ||
            (/\b(jwt|token|session)\b/i.test(msg) && /\b(expired|invalid)\b/i.test(msg));

          if (looksExpired) {
            setAuthError(true);          // <-- mark that this is an auth problem
            openSessionExpired();        // <-- open your SweetAlert
            setProfile(null);            // don't render profile
            return;                      // IMPORTANT: stop here so CTA isn't shown
          }
    ...
    if (!profile) {
      // If the reason is "auth expired", just show the modal (no CTA behind it)
      if (authError) {
        return (
          <SweetAlert
            open={alert.open}
